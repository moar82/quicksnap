...
class ZoomIndicator extends AbstractIndicator {
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            BitmapTexture tex = CanvasTexture.newInstance(
    ...
    protected BitmapTexture getIcon() {
        if (mDrawIndex != mZoomIndex) {
            ...
            mTitle = CanvasTexture.newInstance(
                    sZoomFormat.format(value), mFontSize, FONT_COLOR);
        }
        return mTitle;
    }
    ...
    public GLView getPopupContent() {
        if (mZoomController == null) {
            ...
            mZoomController.sFineTickMark.setAvailableZoomRatios(mZoomController, mZoomRatios);
            ...
            mZoomController.sBackground.setZoomListener(mZoomController, new MyZoomListener());
            mZoomController.sBackground.setZoomIndex(mZoomController, mZoomIndex);
        }
    ...
    public void setZoomIndex(int index) {
        if (mZoomIndex == index) return;
        if (mZoomController != null) {
            mZoomController.sBackground.setZoomIndex(mZoomController, index);
        } else {
